...
export default function Analyze({ isDark }) {
  ...
  // Simplified and fully decoupled background styles for proper thematic handling
  ...
  const modes = [
    { id: 'text',  label: 'Analyze Text', icon: <TypeIcon  size={16} /> },
    { id: 'image', label: 'Verify Image', icon: <ImageIcon size={16} /> },
    { id: 'audio', label: 'Check Audio',  icon: <Mic       size={16} /> },
  ];
    ...
    <div className="w-full flex justify-center pb-8 px-4 md:px-8 md:-mt-12">
      <div className="w-full max-w-6xl flex flex-col items-center">
        ...
        <div className="flex p-1 gap-1 rounded-full mb-6 border border-surfaceBorder bg-surface shadow-sm transition-colors">
          {modes.map((m) => (
            <button key={m.id} type="button"
              ...
              className={`flex items-center gap-2 px-5 py-2 rounded-full text-xs md:text-sm font-bold transition-all ${mode === m.id ? 'bg-primary text-background shadow-md' : 'text-muted hover:bg-surfaceBorder'}`}>
              {m.icon} {m.label}
            </button>
          ))}
        </div>
        ...
        <motion.div className="w-full max-w-5xl border border-surfaceBorder rounded-3xl bg-surface backdrop-blur-3xl shadow-xl overflow-hidden ring-1 ring-surfaceBorder/50 mb-6 transition-colors">
          <div className="bg-surfaceBorder/20 border-b border-surfaceBorder px-5 py-3 flex items-center justify-between">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-6 p-6 md:p-8 relative">
            ...
            <div className="flex justify-end mt-2">
              <button type="submit" disabled={isDisabled}
                ...
                {loading ? <><Loader2 size={18} className="animate-spin" /> Analyzing...</> : <><Search size={18} /> Verify {mode === 'text' ? 'Truth' : mode === 'audio' ? 'Audio' : 'Image'}</>}
              </button>
            </div>
          </form>
        </motion.div>
        ...
        <AnimatePresence>
          {error && (
            <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
              className={`w-full max-w-5xl ${error.includes('Note:') ? 'bg-primary/5 border-primary/10 text-primary' : 'bg-red-500/10 border-red-500/20 text-red-500'} rounded-xl p-4 flex items-center gap-3 mb-6 shadow-sm block`}>
          ...
          {result && !loading && (
            <motion.div ref={resultRef} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }}
              ...
              <div className="p-6 md:p-10 flex flex-col gap-8 bg-surface">
                ...
                {/* 3. Database Block (Keeping the user's commented section exactly as they had it) */}
                {/* <div>
                  <h3 className="text-[10px] font-black text-muted uppercase tracking-widest mb-3 flex items-center justify-between select-none border-b border-surfaceBorder pb-2">
                    Historical Database
                    <span className="text-[10px] bg-primary/10 text-primary px-2 rounded-full">{result.sources?.qdrant?.length || 0}</span>
                  </h3>
                  {result.sources?.qdrant?.length > 0 ? (
                    <div className="flex flex-col gap-2">
                      {result.sources.qdrant.map((q, i) => (
                        <div key={i} className="px-3 py-2 rounded-lg border border-surfaceBorder bg-background/50 flex items-center justify-between gap-2">
                          <span className="text-xs text-muted truncate italic flex-1">"{q.claim}"</span>
                          <div className={`px-1.5 py-0.5 rounded text-[8px] font-black uppercase ${q.label === 'TRUE' ? 'bg-green-500/20 text-green-500' : 'bg-red-500/20 text-red-500'}`}>{q.label}</div>
                        </div>
                      ))}
                    </div>
                  ) : (
                    <p className="text-xs italic text-muted">No matches in verified Qdrant dataset.</p>
                  )}
                </div> */}
